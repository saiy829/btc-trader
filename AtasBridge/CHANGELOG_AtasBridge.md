...
## v2026.08.13-2（2026-08-13，SweepMarker：分批离场口径 + TP1 可配）

### 起因：365 天样本推翻了 28 天样本的结论

Sea 把图表历史拉到约 365 天，样本从 31 笔涨到 **469 笔**：

| | 已结 | 触及TP1 | 到达TP2 | 止损 | 期望 | 亏损单最大浮盈 |
|---|---|---|---|---|---|---|
| 做多 | **252** | 41%(104) | 26%(65) | 74%(187) | **+0.01R** | 0.8R |
| 做空 | **217** | 43%(94) | 28%(60) | 72%(157) | **+0.10R** | 0.9R |

合计约 **+0.05R/笔**，基本为零。

**28 天那份（做多 +0.49R / 做空 −0.13R）是小样本噪声**，而且方向判断也反了 ——
当时的结论「亏损全部来自做空」在 31 笔上看着成立，469 笔上不成立，现在做空
反而略好。本文件 `v2026.08.13-1` 条目中基于 31 笔样本给出的方向性结论，
以本条为准。

从期望值反推平均盈亏比约 **2.95R**，对应盈亏平衡胜率 `1/(1+2.95)=25.3%`，
实际到达TP2 26~28% —— 只高出 1~3 个百分点，边际极薄。

### 数据里的可量化改进点

触及TP1 41%/43% 而到达TP2 仅 26%/28%，差额即「**摸到 TP1 后又被打回止损**」：
做多 104−65=39 笔，做空 94−60=34 笔，合计 **73 笔，占 469 的 15.6%**。
当前的全进全出口径把这 73 笔记为完整 −1R，**低估了这套 setup 的真实价值**。

### 新增「分批口径」统计（精确计算，不是建模）

面板新增一行：

```
分批口径(TP1减半仓+移保本)：做多 +0.XXR　做空 +0.XXR　合计 +0.XXR　（共 469 笔，TP1=1.5R）
```

**这不是带假设参数的模型，而是可精确计算的**：做多的止损在入场下方、TP1 在
入场上方，一笔"触及 TP1 后止损"的单子必然回穿过入场价，因此剩余半仓就是在
保本位离场，不是 −1R。做空镜像同理。三种情形：

- 到达TP2：`0.5×TP1倍数 + 0.5×盈亏比`
- 触TP1后止损：`0.5×TP1倍数 + 0.5×0`
- 直接止损：`−1R`

### TP1 倍数开放为设置项（原先硬编码 1.5）

469 笔样本显示**亏损单平均最大浮盈 0.8~0.9R** —— TP1 放在哪里直接决定这部分
能兑现多少。原来 1.5 是写死的常量，现在是设置项「TP1倍数(R)」，默认仍为 1.5，
信号线上的 TP1 标签也跟着显示实际倍数。

### 部署

双平台 0 错误，各 125952 字节。

---

## v2026.08.13-3（2026-08-13，SweepMarker：计入交易成本的净期望）
...
```
全进全出：合计 +0.05R
分批口径：做多 +0.08R　做空 +0.16R　合计 +0.12R　（共 469 笔，TP1=1.5R）
```

**+0.05R -> +0.12R，翻了 2.4 倍**，证实「摸到 TP1 后被打回」那 73 笔确实被
全进全出口径严重低估。至此可以确定：**离场纪律对这套 setup 的影响大于入场调参**。

### 但整段迭代都漏掉了交易成本，而它足以改变结论

仓位 = 权益 x 风险% / R，所以手续费占 R 的比例只取决于 **入场价 / R**：

```
成本(R) = 2 x 单边费率 x 入场价 / R
```

Sea 样本里 R 的中位数约 205 美元、入场价约 64000：

| 口径 | 成本 | 分批毛期望 | 净期望 |
|---|---|---|---|
| 仅 taker 0.045%、零滑点 | 0.28R | +0.12R | **-0.16R** |
| taker + 0.005% 滑点（合 0.05%/单边） | 0.31R | +0.12R | **-0.19R** |

**光手续费就吃掉 0.28R，而毛边际只有 0.12R。** 扫除反转位是插针那一刻市价进场，
滑点只会更大。结论：**当前配置作为独立系统是负期望的。**

分批离场不额外增加成本：入场满仓、出场 0.5+0.5，成交量仍是 2 倍仓位。

### 新增设置项与统计

- **「单边成本(%,含手续费与滑点)」**，默认 0.05。设 0 则只显示毛期望。
- 方向统计行改为 `毛期望 X.XXR　成本 -X.XXR　净期望 X.XXR`
- 分批口径行追加 `|| 扣成本后：做多 … 做空 … 合计 …`

以后每次测量都自带净期望，不会再出现"毛期望正、以为能做"的误判。

### 由此得到的方向性判断

净期望为负的原因不是胜率不够，而是 **R 相对价格太小**：R 中位数 205 美元
只有价格的 0.32%，成本占 R 的比例因此高达 28%。三条可能的出路，按预期收益排序：

1. **筛大 R 的信号**。`最大止损距离` 上限 0.8% 对应 R≈512 美元，成本降到 0.11R，
   净期望即可转正。可通过提高「最小止损距离」实现。
2. **限价进场**替代市价（在池回踩位挂单），把 taker 换成 maker。
3. **找出正期望子集**（按 ADR 分档、合并数 >=2、非弱信号分别统计）。指标目前
   不做分档统计，需另开卡。

### 部署

双平台 0 错误，各 126976 字节。字符串校验方法一并修正：此前用
`Encoding.Unicode.GetString(bytes)` 从偏移 0 解码整个 dll 来找字符串常量，
而 #US 堆里的常量可能落在奇数字节偏移上，导致解码错位、校验假阴性。
现在偏移 0 与 1 都试。

---

## v2026.08.13-4（2026-08-13，任务卡 9H：高周期方向过滤）

规格见 `reports/SWEEP_9H_20260813.md`。9G 的终局结论只穷尽了**成本侧**四条路，
本卡是第一次探索**毛边际侧**。

### 起因

Sea 复盘 M15 一笔亏损单（入场 63837 / 止损 63630.1 / R=206.9 / TP2 64470(3.1R)
/ 结果止损）时问「这个地方做多的原因是什么，指标是否考虑大的时间框架」。

那笔的触发链条逐条成立（SSL 被击穿 → Delta 落在最低5%分位 → 放量 → 3根内收回
→ ADR≤0.8 → 有吸收），**指标没做错**。问题是它发生在价格从 64470 崩下来之后的
第一次反弹，而 `TP2=64470` 正是那根刚崩掉的高点 —— **那不是扫止损，是真突破**。
指标区分两者的手段（最大穿透、ADR）**都是局部的单周期度量**，看不到大方向已转空。

数据一直在暗示：作废原因 **29/31 是「穿透过深」**（= 大多数"扫除"其实是真突破）；
469 样本做空 +0.16R > 做多 +0.08R，M15 样本却反过来（做多 +0.15R > 做空 −0.12R）
—— **方向优劣随行情翻转，正是缺少方向过滤的特征**。

### 需要多大改善才值得

成本地板 0.125R、当前毛边际 +0.11R，要让净期望有意义需毛边际约 0.25R。
按平均盈亏比 2.95R 反推：`0.25 = p×2.95 − (1−p)` → **p ≈ 31.6%**，
而当前 TP2 命中率 26~28%。**只要提高 4~6 个百分点就能翻正。**

### 设计：测量优先，过滤可选

关键决策是**不把过滤做成"开了才能测"的开关**：

1. 永远计算高周期方向
2. 每笔信号确认时打上标签（顺势/逆势/中性）
3. **统计按标签分三组显示**
4. 过滤开关**只控制画线与提示音**，不影响统计

**加载一次即可同时读到三组净期望**，无需反复切设置对比 —— 这直接针对 9G 反复
出现的「改了参数不知道是哪个改动起作用」。

### 实现

- **聚合**：沿用 `UpdateDailyAtr()` 的手法，把图表K线按 `HtfMinutes` 分桶聚合成
  高周期K线。**只有已收盘的桶进入 `_htf`**，正在形成的桶不参与判定。
- **摆动点**：在聚合序列上跑与主逻辑同一套检测（`ScanHtfPivots()`），
  需右侧 N 根高周期K全部收盘才确认。
- **结构判定**：最新摆动高>前高 且 最新摆动低>前低 → 多头；反之空头；其余中性。
  刻意从严 —— 区间震荡应读作中性，而不是来回翻转。
- **无前视**：聚合在 `ProcessClosedBar()` 中按时间顺序增量构建，天然因果。

### 新增设置项（分组「7 高周期过滤」）

| 参数 | 默认 | 说明 |
|---|---|---|
| 高周期(分钟) | 60 | 60=H1，240=H4 |
| 高周期摆动点左右K数 | 3 | 调大=结构更粗更稳 |
| 方向过滤模式 | 0 | 0=关闭 1=只顺势 2=只逆势 |
| 中性时放行 | true | 结构不明确时是否放行 |

「只逆势」不是笔误，是**对照组** —— 若逆势子集显著更差即证实假设；若两者差不多，
说明方向过滤这条路也不通，可以及早收手。

用 `int` 而非 enum：ATAS 直接显示 enum 成员名，而中文标识符会破坏「代码用英文」
的约定，故把含义写进 Description。

### 面板新增四行

```
高周期(60分钟)结构：多头结构(HH+HL)　过滤模式：关闭（统计始终三组全计，过滤只影响画线与提示音）
顺势：已结 N 笔  触及TP1 xx%  到达TP2 xx%  全进全出 +x.xxR  分批 +x.xxR  成本 -x.xxR  分批净 +x.xxR
逆势：…
中性：…
```

信号标签与结算日志也追加了顺势/逆势/中性标记。

### 验证方式

365 天 M5、`方向过滤模式=0`，加载一次读三行分组统计：

| 结果 | 结论 |
|---|---|
| 顺势毛边际 ≥ 0.25R | 假设成立，9G 终局结论需改写 |
| 顺势明显 > 逆势但未达标 | 方向有用但不够，需叠加其它过滤（另卡） |
| 顺势 ≈ 逆势 | 假设不成立，方向过滤无效，及早收手 |

### 部署

双平台 0 错误，各 132096 字节。

---

## v2026.08.13-6（2026-08-13，设置项改动强制重算 —— 修一个会让所有测量失效的坑）

### 症状

Sea 把「最小止损距离」设为 0.5%、「方向过滤模式」设为 3（只区间），面板显示：

```
做多：已结 17 笔 ... 毛期望 +0.49R  成本 -0.39R  净期望 +0.10R
高周期(60分钟)结构：空头结构(LH+LL)　过滤模式：只区间
```

### 用算术证明这批数字是旧的

成本恒等式 `成本(R) = 2 × 单边费率 ÷ 止损距离%`。若 0.5% 的下限真的生效，
则所有信号止损 ≥ 0.5%，成本必然 **≤ 2×0.05%/0.5% = 0.20R**。

面板显示 **0.35~0.40R**，反推平均止损距离仅 **0.26%** —— **低于所设下限**。

**结论：统计里的信号是按旧参数算出来的。** 而面板上「过滤模式：只区间」是
`DrawPanel()` 实时读属性值渲染的，所以标签立刻变了、数字却没变。这种"标签新、
数据旧"的组合最危险 —— 看起来一切正常，实际每个数字都不可信。

### 根因与修法

设置项原本都是自动属性（`{ get; set; }`），改动后 ATAS 未必触发完整重算。
反射确认 `ExtendedIndicator.RecalculateValues()` 可用（protected），
于是把**所有影响信号生成的 26 个设置项**改为显式属性，setter 统一走：
...
覆盖范围（26 项）：池识别 3 项、扫除检测 6 项、确认 5 项、交易 7 项
（含 TP1倍数 / TP2最大R / 账户权益 / 单笔风险 —— 后两者影响落库的仓位数字）、
高周期过滤 4 项。

**纯显示项不走 SetCalc**（作废标记/池线/延伸K数/颜色/字体/线条文字/胜率统计/
声音开关与文件名）—— 它们不改变信号生成，ATAS 自身的重绘足够。
`单边成本(%)` 也不走：它只参与面板里的实时计算，不影响已落库的信号。

### 附带得到一个自检手段

`成本 ≤ 2 × 单边费率 ÷ 最小止损距离` 是硬算术界。以后只要看一眼面板的成本数字
就能判断「最小止损距离」有没有真正生效 —— 例如设 0.5% 时成本若超过 0.20R，
说明读到的是旧数据。

### 部署

双平台 0 错误，各 131584 字节。

---

## v2026.08.13-7（2026-08-13，任务卡 9I：限价进场 + 三组并行统计）

规格见 `reports/SWEEP_9I_20260813.md`。9H 结束在净期望 +0.00R（分批毛 +0.17R、
taker 成本 0.16R），三个杠杆中只剩限价进场未做。

### 核心设计：三组并行记账

限价进场**改变的不只是成本，还有样本构成** —— 未成交的信号从盈亏统计里消失，
而它们不是随机消失的（只在回踩时成交，系统性偏向某类行情）。若不记账会得出
方向未知的假结论。

因此每个确认信号同时模拟两条路径，面板输出三组：

- **A 限价成交**：路径L成交者，maker 费率 + 限价 R 结算
- **B 限价未成交**：路径L未成交者，但**显示其在市价口径下的结果** ← 判断漏掉的
  是好单还是坏单
- **C 全市价基线**：全部信号走市价路径 = 9H 口径，作对照

`B 优于 A` = 限价漏掉好单；`B 劣于 A` = 双重收益；`A ≈ B` = 纯成本收益。
**三组同时显示，不靠切参数分别测** —— 9H 的教训。

### 限价规则

```
挂单价 = 确认K收盘 ∓ 限价回撤R × R_market
```

挂单价越过止损则判「限价无效」并单独计数（`限价回撤R ≥ 1.0` 时必然发生）。

成交判定**从确认K的下一根开始**（确认K的低点在信号成立前就已形成，用它判成交
是前视偏差），**用 Low/High 而非收盘价**（挂单盘中被触及即成交）。填单那根K
本身也参与结算，沿用「同一根K同时触及止损与目标按止损计」的保守规则。

成交后 R 重算：`R_limit = 挂单价 − 止损`，TP1 随之重算，TP2 价格不变但以
R_limit 计的倍数变大。

### ⚠ R 变小会部分抵消 maker 的优势（必须写进报告的一点）

`成本(R) = 2 × 费率 × 价格 ÷ R`，R 变小则成本占比上升。按 9H 实测均值
（入场 ≈ 63700，R_market ≈ 398 即 0.625%）估算：

| 口径 | R | 成本 |
|---|---|---|
| 市价 taker 0.05% | 398 | 0.160R |
| 限价 maker 0.023%，回撤 0.3R | 279 | **0.105R** |

实际降幅约 **34%**，而非只看费率差得出的 54%。净期望的算术预期应为
`+0.17 − 0.105 ≈ +0.065R`，**不是 +0.10R**。这正是必须实测而非只算的原因之一。

### 新增参数（分组「8 限价进场」）

进场模式(0市价/1限价)=1、限价回撤(R)=0.3、限价超时(根)=3、
maker费率(%)=0.023；原「单边成本(%)」移入本组并更名为「taker费率(%,单边,含滑点)」。
费率做成参数是因为交易所等级会变，写死会让成本计算失真。

前三项走 `SetCalc()` 强制重算；两个费率不走（只参与面板实时计算，
不改变已落库的信号）。

### 绘制

限价模式下入场线画在挂单价；已成交实线并标「限价成交」，R 与盈亏比按 R_limit
显示；未成交/挂单中画**半透明虚线**且不画止损与目标线（避免与真实成交混淆）。

### 未动的部分

市价路径的所有字段完全未修改，因此 **C 组严格等于 9H 基线**，可直接对照。
池识别、两阶段检测、作废逻辑、止损/TP 计算方式均未改动。
