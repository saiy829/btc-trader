...
| 2026-07-26 | Bug#40+策略证否：(1)修复atas_bars ×10脏价格——2026-07-13 10:14~07-16 21:34约3.5天AtasBridge逐字段(非整行)把价格推成×10尺度(open/high/low/close/poc等,四路各约996行共3981行,非持续,前后皆干净)，污染tracker前瞻收益(4.8% outcome出现±900%不可能值)；逐字段>150000则÷10修复(混合行只改被污染字段)，整库备份db_before_x10fix+快照表atas_bars_x10bak可回滚。(2)两次离线回测结论:综合分引擎=ETF单维符号跟随(46信号全部ETF主导)负期望-0.31R/单、任何确认门(P1)最多磨到-0.11R仍负且过拟合;吸收信号方向(bid/ask)在清洗后数据上漂移中性边际≈0(均值+0.005%),即便按区间位置分档,预测力全来自位置(低位+2.4%/高位-0.8%=均值回归,regime依赖)而非吸收方向。(3)P3:综合分引擎新增PUBLISH_TG=False停发TG(避免推误导性纸面信号),引擎继续评分/记engine_signals+scores/跟踪结果供研究,不动任何评分触发逻辑。战略结论:两套自动信号源(宏观综合分+订单流吸收)经验证均无方向alpha,系统价值定位应为'给Sea酌情交易提供数据背景/简报',而非自动发信号 |
| 2026-07-26 | 全数据管线完整性审计（Sea要求：错误数据只能得错误结论）——系统性核查后确认：(A)atas_bars四路价格×10修复后与独立源mark_px比值中位1.0000、0%偏离超2%，OHLC零违规、尺度全对；(B)宏观表费率(小数)/OI(60-71亿)/多空比(1.0-1.7)/结构 尺度全对、3分钟新鲜；(C)ETF双独立源(Farside/SoSoValue)近12日吻合到±0.1M、幅度合理——唯一主导维度已事实校验；(D)volume/delta尺度自洽(|delta|≤volume零违规、ask+bid=volume零不一致、OKX张→BTC×0.01正常)，×10窗口量能高2.8×判定为真实高波动非bug；(E)四路身份无串、时间对齐经跨源比值1.0000反证。发现残留：5处数据缺口(最大07-16 23.7h,属缺失非错误)。收尾修复：atas_signals 6355个×10价(price列+raw_payload)还原、atas_signal_outcomes 24020条脏change重置由tracker用干净bar重算(4H均值从+43.9%垃圾回+0.532%正常)。结论：修复+审计后价格/宏观/ETF/量能数据全部验证干净，此前'两套自动信号源均无边际'的结论建立在可信数据上成立(引擎用Binance实时价+已验证宏观维,吸收回测用修复后atas_bars,均未触及污染路径)。项目病根定性：攒数据快于验数据，一串数据完整性bug(#33/#37/#38/#40)长期无声毒化分析——数据验证纪律应先于任何策略结论 |
| 2026-07-26 | 简报P0：综合信号分降格为背景+去交易计划gate——回测证否综合分无边际后,简报仍在每会话把它当权威方向信号引用、还用|综合分|>30 gate早盘保守档,属'把噪声当信号呈现'。纯prompt层改动(仅ai_analyst/briefing.py,不碰utils/signal_score.py与采集):SIGNAL_BLOCK由'综合信号分权威数值'改为「六维市场背景读数」(六维原始强度保留为背景数据,明确标注非信号+禁止加权求和当方向依据+附无边际caveat);早盘第1节评级改为综合市场背景判断不依赖综合分;早盘保守档去掉|综合分|>30 gate改为'市场结构清晰(IB/VP/流动性一致)';欧美盘按需的综合分环比引用改为六维背景一句话。compute_and_save保留继续写signal_scores历史。确定性验证:prompt含'六维市场背景读数'、不含'综合信号分'/方向标签;实跑ondemand end_turn正常发布。定位对齐:简报从'替你报没用的信号+点位'回归'给你干净市场背景,扳机你按'——与Sea酌情AMT/订单流方法论一致 |
| 2026-07-26 | 简报P1：模型升级(claude-sonnet-4-5→sonnet-5/opus-5)——早盘/周报=claude-opus-5(深度分析,保留扩展思考,max_tokens提到16000给思考留预算,实测11403字end_turn未截断,耗时~4min);其余频繁短会话(noon/europe/evening/ondemand)=claude-sonnet-5(原token预算)。踩坑记录:sonnet-5/opus-5默认对复杂prompt开扩展思考,思考token计入max_tokens——(1)opus返回ThinkingBlock,原msg.content[0].text会AttributeError崩→改健壮取文本只取type=='text'块跳过思考块;(2)短会话小预算(2000-3000)被思考吃光导致正文截断(实测evening sonnet-5仅247字max_tokens截断)→短会话显式thinking={type:disabled}关思考,恢复可预测(evening改后1445字end_turn)。按会话配置模型+token+思考开关集中在ai_analyst/briefing.py顶部MODELS/MAX_TOKENS,想调回sonnet-5只改一行。教训延续:升级API不能只改model字符串,必须端到端实跑验证(新模型思考行为差异会静默截断) |
| 2026-07-26 | 面板新增「多时间框架结构」实时读盘卡片——起因Sea指出项目在摊大饼、有数据却不用数据分析市场。新建 briefing/market_structure.py::get_market_structure()：自上而下输出 周(开高低/涨跌/区间分位/周VP POC-VAH-VAL/VA状态) → 昨日(PDH-PDL-PDC/昨VP) → 今日(开高低/日内分位) → 近4H(价格结构+Delta/CVD/正负bar数/价量背离判定/4H VP/高频POC) → 足迹吸收带(4H聚合,比值≥2且量≥5) → 近8H大单净向 → 宏观(象限24H分布/费率Z/OI/多空比/ETF稳定视图) → 关键价位表(全时间框架汇总,同价位标签合并,标注上下方位与距离%)。设计原则:纯计算无AI(可高频刷新/零API成本/不会编数)、只描述不预测(回测已证否本系统方向预测力,面板给背景扳机由用户按)、各块独立try/except宁缺勿假。api/main.py 纯新增只读端点 GET /api/market/structure(不动任何既有端点,遵Bug#19)。web/index.html 新增 mtf-panel 卡片(grid加一行、四时间框架盒+横向滚动关键价位条+宏观条,60秒轮询),并按既有部署缺口手动 cp 到 /www/wwwroot/mb.661688.xyz/。已浏览器实测线上渲染正常、数据与手工分析一致 |
| 2026-07-26 | 面板三项调整(Sea要求)：(1)删除引擎信号卡片(sig-panel)——TG已停推,面板也不再展示无边际的信号点位;grid去sig行、删CSS/HTML/JS(refs/pollSignals/sig辅助函数/轮询/return暴露),浏览器实测无console报错Vue未崩。(2)MTF卡片明示数据源为「币安 BTCUSDT 永续」(本就取atas_bars binance/perp,加标识消歧义)。(3)VP口径明示——POC/VAH/VAL升级为基于footprint逐档成交量的真·Volume Profile(原为bar典型价近似),卡片标题+各处标注'成交量Volume Profile(非TPO)',与基于时间的TPO/Market Profile区分清楚(footprint 6884/6919 bar完整,周POC从近似65100变真实64100)。性能:footprint版全量算~2.4秒,加'按最新bar时间戳缓存整份结果'(bar每5min更新,两次间1ms命中)+ETF 30min缓存;端点由async改同步def让FastAPI丢线程池,避免2.4s重算阻塞事件循环(WebSocket/其它端点)。备份panel_before_delsig_* |
| 2026-08-02 | 面板加AtasBridge推送健康指示器+修回归——起因:发现AtasBridge订单流断流~25h(08-01 15:14~08-02 16:xx,Sea ATAS关了),而人只能靠手动查才发现。新增 briefing/market_structure.py::get_feed_health()(每次实查不走缓存——关键:MTF端点按最新bar时间戳缓存,断流时缓存冻结在'新鲜'反而报不出,故健康检查必须绕缓存)以币安永续bar新鲜度为主判据(>10min warn/>20min down/<10 ok),大单/吸收作辅助;新增只读端点GET /api/feed/health(同步def);web/index.html header常驻'推送 ●X分钟前'指示器(绿/黄/红,红闪烁,hover看bar/大单/吸收各自时效),30秒轮询。顺带修复一个回归:之前'删信号卡'补丁把紧邻的MTF CSS也一起删了(.mtf-计数曾=0,MTF卡片裸渲染几天),本次一并恢复27条MTF样式。均浏览器实测无console报错、'推送 1分钟前'正常显示 |
| 2026-08-02 | Phase 8 第二步:订单流信号引擎(反转承接v1)——'实现结果'路径:数据修复后建生成器,让entry/stop/tp从AtasBridge订单流长出来,而非已证否的宏观综合分。新模块monitor/of_engine.py+新表of_signals+新服务btc-of-engine,与signal_engine彻底分开。v1规则(Sea选定反转承接):做多=现价在4H区间下1/3+贴近参考支撑(周VAL/PDL/昨POC容差0.15%)+该处强买方吸收(footprint逐档bid/ask>=3且>=20BTC);做空镜像;入场=现价,止损=吸收带/结构下方-0.3ATR,止盈1R/2R。复用market_structure做结构读取、signal_engine验证过的ATR/结果跟踪(stop/t1/t2/expire,无条件每轮先跑-Bug#38)/冷却拦生成(P0)。AtasBridge显示:把它轮询的/api/signal/history改指向of_signals(补score=0兼容DLL),零改DLL图上即画订单流信号;综合分engine_signals不再显示(仍在库,signal_engine可停)。全程纸面观察不接下单。实测:引擎跑2h每5min评估无报错;规则首次真实触发#1 LONG(低位15%+支撑$63050+买方吸收3.4x/38BTC),依据透明,端点正常喂出。下一步:跑够样本用of_signals的胜率/R判断v1有无边际,再迭代规则 |
| 2026-07-26 | 每日简报暂停闸(Sea要求暂停发送)：scheduler.py 4个定时任务(早盘/正午/欧盘/美盘,含周一周报)顶部加哨兵检查——存在 /opt/btc-trader/BRIEFING_PAUSED 文件则跳过,不生成不发TG/WP。仅拦定时简报;/b手动简报与/pos命令不受影响,btc-briefing服务照常运行。暂停:touch BRIEFING_PAUSED; 恢复:rm BRIEFING_PAUSED——每次触发实时检查,无需重启且扛服务重启。哨兵文件属运行状态不进git(.gitignore或本就在项目根,恢复只删文件)。当前已touch=暂停中 |
