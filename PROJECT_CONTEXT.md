...
已知设计缺口（未来校准用）：三因子状态实际系统会产出14种细分标签
（如"多头被迫平仓⬇️""横盘多头拥挤⚠️"等），但 SCORE_CONFIG.regime_map 只定义
了6个关键词类别（真实建仓/健康趋势/混合信号/挤压酝酿/过热/去杠杆清洗）的分数。
未覆盖的7种细分标签统一按"混合信号"处理记0分，这是保守处理，不是遗漏，
Phase 5B 拿到胜率数据后可重新校准这7种的分数。
