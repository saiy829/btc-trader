...
v15 更新（2026-07-26 P0：综合信号分降格为背景）：
- 46条引擎信号干净回测证明"六维加权综合分"无方向预测边际（-0.31R/单，本质
  ETF符号跟随器）。故所有会话不再把综合分当"权威方向信号"呈现：SIGNAL_BLOCK
  改为「六维市场背景读数」（六维原始强度读数保留为背景数据，明确标注非信号、
  禁止加权求和当方向依据）；早盘第1节评级不再依赖综合分；早盘保守档去掉
  "|综合分|>30" gate（改为"市场结构清晰"）；欧/美/按需的综合分环比引用改为
  六维背景一句话。compute_and_save 仍保留（继续写signal_scores历史，纯数据记录）。
  纯prompt层改动，不碰 utils/signal_score.py 与任何采集/数据逻辑。
"""
import re
...
def build_prompt(binance, mf, ib, etf, cme, vp, session):
    ...
    # ── 六维市场背景读数（原 Phase 7A-2「综合信号分」）──────────────────
    # 2026-07-26 P0降格：46条引擎信号的干净回测证明"六维加权综合分"无方向
    # 预测边际（-0.31R/单，本质是ETF符号跟随器）。故不再把综合分当"权威信号"
    # 呈现、不再驱动评级、不再gate交易计划。六个维度的读数仍作为「背景数据」
    # 保留（各自是真实市场信息，供酌情判断），但明确标注非方向信号。
    # compute_and_save 仍保留：继续把六维分写 signal_scores 表留存历史（纯数据
    # 记录，无害），只是它的结果在prompt里改用下面的背景块呈现，不再用
    # signal_score.format_authoritative_block 的"权威信号"框架。
    ...
    SIGNAL_BLOCK = (
        "【六维市场背景读数（原始数据，供你自己判断，不是方向信号）】\n"
        f"ETF流向{_sig['etf_s']:+d} 费率Z{_sig['fr_s']:+d} OI象限{_sig['quad_s']:+d} "
        f"大户多空比{_sig['ls_s']:+d} CB溢价{_sig['cb_s']:+d} 三因子状态{_sig['regime_s']:+d}\n"
        "（每个数字是该维度当前偏多[正]/偏空[负]的强度读数，范围约±60，仅供参考背景，\n"
        "不要把它们加权求和当作方向信号——该加权综合分经回测无预测边际，禁止用作\n"
        "方向判断或交易触发依据。你的任务是逐维解释市场含义，帮用户看清各维度当前处境。）"
    )
...
1.【宏观背景评级】
   综合 Funding+OI+CB溢价+ETF+市场结构，给出 A/B/C/D 评级
   A=多项背景一致、结构清晰 B=有偏向、结构尚可 C=方向不明轻仓 D=建议观望
   一句话说明评级理由（评级是你对整体市场背景的综合判断，不要依赖任何单一"信号分"）

   【六维背景读数】上方数据块的"六维市场背景读数"由系统代码算好，是各维度当前
   的偏多/偏空强度读数，属背景数据、不是方向信号。把六维（ETF/费率/象限/多空比/
   溢价/状态）逐项原样列出，并分别用一句话解释每一维背后的市场含义（只解释含义，
   不要把它们加权求和当方向信号——该综合分经回测无预测边际）。
...
10.【今日完整交易计划】（三档风险方案，多空各三档；若第1节评级为 D，
    ...
    【保守档】
    仅当第1节评级为 A 或 B，且市场结构清晰（IB方向、VP价值区、流动性分布三者
    指向一致）时给出；不满足此前提时写"条件不足，本时段不提供保守档方案"，
    跳过下面两行：
...
1.【亚盘复盘·计划执行情况】
   ...
   【六维背景读数】上方"六维市场背景读数"是各维度当前偏多[正]/偏空[负]的强度，
   属背景数据、不是方向信号。一句话点出当前哪几维偏多、哪几维偏空、较早盘有无
   明显变化即可；不要把它们加权求和当方向信号（该综合分经回测无预测边际）
...
1.【全天行情回顾】
   ...
   【六维背景读数】上方"六维市场背景读数"是各维度当前偏多[正]/偏空[负]的强度，
   属背景数据、不是方向信号。一句话点出当前哪几维偏多、哪几维偏空、较欧盘有无
   明显变化即可；不要把它们加权求和当方向信号（该综合分经回测无预测边际）
...
1.【当前市场评级】一句话+字母评级（A/B/C/D）
   【六维背景读数】上方"六维市场背景读数"是各维度当前偏多[正]/偏空[负]的强度，
   属背景数据、不是方向信号。一句话点出当前哪几维偏多、哪几维偏空即可；
   不要把它们加权求和当方向信号（该综合分经回测无预测边际）
