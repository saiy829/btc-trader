...
1.【宏观背景评级】
   ...
   【综合信号分】在 A/B/C/D 评级之外，另给出 -100（极度看空）~ +100（极度看多）
   的综合信号分，需展开六维拆解，每维独立给出 -100~+100 数值，权重如下：
     - ETF流向（权重25%）：基于最新净流入/净流出方向及本周累计趋势
     - 资金费率Z-score（权重15%）：Z值越极端，该维度分数越倾向于反向修正
       （防止追涨杀跌，Z>+2 应给负分警示，Z<-2 应给正分警示）
     - OI象限（权重20%）：真实建仓（Q1/Q2类）强化同向分数，挤仓/平仓减弱或反向
     - 大户多空比（权重15%）：偏离50%越多同向分数越高，但极端拥挤需适度收敛
     - CB溢价（权重10%）：正值增强做多分数，负值增强做空分数
     - 三因子市场状态（权重15%）：健康趋势/真实建仓加分，过热/挤压酝酿减分
   六维加权求和四舍五入得到综合信号分，按以下格式纯文本单行输出（务必照此格式）：
   综合信号分：+35（偏多）｜ETF+60 费率-10 象限+40 多空比+20 溢价+30 状态+20
...
11.【今日完整交易计划】（三档风险方案，多空各三档；若第1节评级为 D，
    ...
    保守档（仅当第1节评级为 A 或 B，且综合信号分绝对值>30 时给出；
            不满足此前提时写"保守档：条件不足，本时段不提供保守档方案"）：
      做多：触发条件 | 入场区间 $XX-$XX | 止损 $XX | 目标1 $XX | 目标2 $XX | 确认信号
      做空：触发条件 | 入场区间 $XX-$XX | 止损 $XX | 目标1 $XX | 目标2 $XX | 确认信号
    稳健档（标准仓位，正常触发确认）：
      做多：触发条件 | 入场区间 $XX-$XX | 止损 $XX | 目标1 $XX | 目标2 $XX | 确认信号
      做空：触发条件 | 入场区间 $XX-$XX | 止损 $XX | 目标1 $XX | 目标2 $XX | 确认信号
    激进档（Kill Zone 扫流动性反手机会，仓位减半）：
...
1.【亚盘复盘·计划执行情况】
   ...
   【综合信号分】给出 -100~+100 综合信号分（不展开六维），并与早盘对比，纯文本单行输出：
   综合信号分：+42（偏多）｜较早盘 +35 走强
...
1.【全天行情回顾】
   ...
   【综合信号分】给出 -100~+100 综合信号分（不展开六维），并与欧盘对比，纯文本单行输出：
   综合信号分：+50（偏多）｜较欧盘 +42 继续走强
...
1.【当前市场评级】一句话+字母评级（A/B/C/D）
   【综合信号分】给出 -100~+100 综合信号分（不展开六维），并与最近一次简报对比，
   纯文本单行输出：综合信号分：+20（偏多）｜较上次简报 +5 略有走强
